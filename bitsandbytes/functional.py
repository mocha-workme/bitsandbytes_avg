...
def quantize_4bit(A: Tensor, absmax: Tensor = None, out: Tensor = None, blocksize=64, compress_statistics=False, quant_type='fp4') -> Tensor:
    ...
    if absmax is None:
        blocks = n // blocksize
        print(f'[quantize_4bit] n : {n} / blocksize : {blocksize} / blocks : {blocks}')
        blocks += 1 if n % blocksize > 0 else 0
        print(f'[quantize_4bit] n : {n} / blocksize : {blocksize} / blocks : {blocks}')
        absmax = torch.zeros((blocks,), device=A.device, dtype=torch.float32)

        print(f'[quantize_4bit] absmax : {absmax}')
    ...
    if compress_statistics:
        offset = absmax.mean()
        print(f'[quantize_4bit] offset : {offset}')
        absmax -= offset
        qabsmax, state2 = quantize_blockwise(absmax, blocksize=256)
        print(f'[quantize_4bit] qabsmax : {qabsmax} / state2 : {state2}')
        del absmax
